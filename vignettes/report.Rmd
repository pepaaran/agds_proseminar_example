...
From the previous figure and table, we can conclude that the downscaling of temperature works quite well for all sites, with RMSE values between 1.5 and 2 degrees Celsius. On average, we predict a temperature that is less than 1 degrees Celsius higher than the measured temperature, as indicated by the bias. Nevertheless, there doesn't appear to be any strong systematic error because the slope is close to 1. 
...
The error may stem from the calculation of VPD from specific humidity (WATCH-WFDEI) and water vapor pressure (WorldClim), or from the fact that the values provided are averaged over large and heterogeneous regions. The performance in, for example, Chamau is better, which hints that there may be a topological or vegetation type component to our error.

This information and study empowers us to mindfully use these data in future research. We are now aware of the limitations of our data processing and downscaling workflow, enabling the critical analysis of future results that will inherit the uncertainty in the daily data with high spatial resolution.
