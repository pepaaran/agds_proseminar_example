...
As an example, we read and show the temperature data for the month of January 1979 only. The raster contains a layer per day, but we plot the monthly average. We also present the January 1979 monthly average values for specific humidity and surface pressure, since VPD is not a variable in the dataset.
...
patchw + plot_annotation(
  title = "Average January 1979 specific humidity and pressure (WATCH-WFDEI)"
)
```
...
For each variable, 12 .tif files are downloaded. Each file from WorldClim contains the monthly average of a single climatic variable across the whole globe, in pixels of 30s (900x900m at the equator and 900x700m in Switzerland). To highlight the magnitude of our downscaling task, it's worth mentioning that each WATCH-WFDEI pixel contains approximately 120 WorldClim pixels.
...
The downscaled maps of both temperature and VPD values are in similar ranges to the 0.5$^o$ original data. The local variations reflect the topography of Switzerland, as we would have expected. Next, we will compare the downscaled temperature and VPD to measurements taken at surface level.
...
From the results above, we can conclude that the downscaling of temperature works quite well for all sites, with RMSE values between 1.5 and 2 $^{o}$C. On average, we predict a temperature that is less than 1 degrees Celsius higher than the measured temperature, as indicated by the bias. There doesn't appear to be any strong systematic error because the slope is close to 1.
