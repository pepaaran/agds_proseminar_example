...
Let's start by reading the temperature data for the month of January 1979 only and see how it looks. The raster contains a layer per day, but we plot the monthly average. Since VPD is not a variable in the dataset, we cannot observe it directly.
...
#### Computing VPD with WATCH-WFDEI data \ref{@vpd_watch}
...
The downscaled maps of both temperature and VPD values are in similar ranges to the 0.5deg original data. The local variations reflect the topography of Switzerland, as we would have expected. Next, we will compare the downscaled temperature and VPD to measurements taken at surface level.
...
From the previous figure and table, we can conclude that the downscaling of temperature works quite well for all sites, with RMSE values between 1.5 and 2 degrees Celsius. On average, we predict a temperature that is less than 1 degrees Celsius higher than the measured temperature, as indicated by the bias. There doesn't appear to be any strong systematic error because the slope is close to 1.
...
The error may stem from the calculation of VPD from specific humidity (WATCH-WFDEI) and water vapor pressure (WorldClim) and the approximation used in section \ref{@vpd_watch}, or from the fact that the values provided are averaged over large and heterogeneous regions. The performance in, for example, Chamau is better, which hints that there may be a topological or vegetation type component to our error.
...
In this project, we sought to downscale monthly Vapor Pressure Deficit (VPD) and temperature data, bridging the gap between coarser datasets and finer temporal scales. Our findings have illuminated significant insights into the effectiveness of our approach, and have also highlighted avenues for refinement and further development.

Our investigations have led us to a dual discovery concerning temperature and VPD downscaling. Temperature downscaling exhibited promising results, showcasing the capability of our methodology to effectively enhance spatial and temporal resolution. However, the same cannot be said for VPD downscaling, which yielded less favorable outcomes. This disparity may stem from the intricate nature of VPD, reliant on multiple meteorological variables, making its accurate downscaling a more complicated task.

This information empowers us to mindfully use these data in future research. We are now aware of the limitations of our data processing and downscaling workflow, enabling the critical analysis of future results that will inherit the uncertainty in the daily data with high spatial resolution.
